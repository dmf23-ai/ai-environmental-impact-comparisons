...
**Phase 6 closed 2026-05-06.** All 14 comparison cards on /comparisons and all 7 home-page displays carry a "How this was calculated →" link to a per-display section in /methods. The half-deployed UX state the prior handoff flagged (six anchor links pointing at non-existent /methods fragments) is closed.

methods.astro contains 21 sections, 78K bytes / 1,806 lines: 7 home-page displays (hourly-hero, range-vs-point-primer, annual-twh, water-bracket, dc-trajectory, ai-share-trajectory, household-equivalents) followed by 14 comparison-card sections in thematic order (water-residential-outdoor → water-vs-golf → electricity-us-households → electricity-ac → electricity-holiday-lights → electricity-gaming → electricity-streaming → electricity-bitcoin → electricity-ev-charging → co2-driving-vs-datacenters → co2-aviation → co2-cement-steel → co2-cattle → co2-lawn-equipment).

Cadence note: methodology pass ran across four sessions grouped by interlock. Session A took the three independent infographics; Session B took the interlocking triad (DC trajectory + AI-share + household-equivalents); Session C took 8 standalone comparison cards; Session D took the 6 overlap comparison cards using a pointer-plus-pairing-rationale pattern — the section's prose handles the editorial framing of the pairing, and the Derivation block points to the canonical /methods section that already carries the math. Assumptions and Uncertainty subsections are dropped on overlap sections; the streaming card is the one hybrid (its annual TWh figure has no canonical home, so streaming keeps a real Derivation block for that figure plus pointer references to the canonical sections for the data-center bar and the per-hour reading).

Next: **visuals #7 and #8** — visual #7 (training vs. inference over time) and visual #8 (watt-scale primer), both parked since Phase 5 wrap pending their own methodology sections. Both are additive to the home-page stack rather than blocking; either could open the next session, or the work could pivot to Phase 7 (verifier script and weekly GitHub Action) per `website_plan.md` Section 9. The visuals path is the natural continuation of the editorial frame; the Phase 7 path is the natural continuation of the trustworthy-figures frame. Decide at the top of the next session.
...
- Session D methodology batch landed 2026-05-06 — six pointer-plus-pairing-rationale sections in /methods (water-vs-golf, electricity-us-households, electricity-gaming, electricity-streaming, electricity-bitcoin, electricity-ev-charging). Pattern for overlap cards: intro paragraph + pairing-rationale paragraph(s) + Derivation block whose body is a pointer link to the canonical /methods section that carries the math + SourceLine. Assumptions and Uncertainty subsections are dropped — they live at the canonical destination and re-stating them under different headings would muddy more than help. The streaming section is the one hybrid: its annual 100–300 TWh figure has no canonical home (annual-twh doesn't carry streaming, the hourly hero only carries the per-hour number), so streaming keeps a real Derivation block for the streaming TWh range and points to annual-twh only for the data-center bar, plus a closing pointer to the hourly hero for the per-hour reading. methods.astro grew from 68K / 1,552 lines to 78K / 1,806 lines.

- Session C methodology batch landed 2026-05-06 — 8 standalone comparison-card sections in /methods plus the `methodsAnchor={anchor}` wiring on `ComparisonCard.astro`. Sections shipped in thematic order: water-residential-outdoor, electricity-ac, electricity-holiday-lights, co2-driving-vs-datacenters, co2-aviation, co2-cement-steel, co2-cattle, co2-lawn-equipment. New schema element for comparison-card sections (vs. infographic sections): an editorial paragraph after the intro that names the pairing rationale, expanded to two paragraphs for cards where the comparison itself does argumentative work (cement-steel, water-residential-outdoor) and collapsed to a single intro for the simpler 2-bar cards. New frontmatter helper `findComparison(id)` parallel to `findInfographic`. Section IDs use the comparison's `anchor` field (e.g., `co2-aviation`), not its `id` field (`co2-aviation-vs-datacenters`).

- Session B methodology batch landed 2026-05-06 — three new sections in /methods (dc-trajectory, ai-share-trajectory, household-equivalents) plus the matching three components now pass `methodsAnchor={data.anchor}` to SourceLine. Trajectory sections walk historical anchors then projection bounds in low → central → high order; household-equivalents walks the conversion factor (11.9 MWh per HH) then each row in display order. The Goldman 1,350 TWh figure is presented as a published forecast rather than derived from the firm's "165 percent increase" framing — the percentage doesn't reconcile to the absolute on any plausible base year.

- HourlyImpactHero.astro confirmed clean on disk during Session B verification — the orphan duplicate-tail flagged in the prior handoff was not actually present. Bash mount showed a single `</style>` close; the Read tool earlier was apparently displaying cached pre-cleanup content. The pattern reinforces the Edit/Write truncation memory's rule: bash mount is ground truth, not the Read tool.
...
- **methods.astro is 78K / 1,806 lines after Session D.** Session D's six-section batch was applied via a Python script written to `outputs/` and executed through bash — the file is modified through the bash mount, not by the Edit/Write tool, and the post-edit dump tax did not fire on the target file in observed runs. The pattern for future dump-tax-heavy edits: draft in chat, write a Python script with sentinel-string anchored replacements to `outputs/`, run via bash, verify with `wc -lc` and `grep` on the bash mount. The script is discardable after the run; `outputs/` is ephemeral.
...
src/
  ...
  pages/
    ...
    methods.astro                          ← per-display methodology page (Phase 6 complete, 21 sections: 7 home-page displays + 14 comparison cards)
